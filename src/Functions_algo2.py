...
def coupe(indice_debut,indice_fin,cours,plus_HautBas, dataDF) :                                         ## METTRE EN PARAMETRE???
...
def pt_interessant_typeDiv(indM,ID,l,l2,plusHautBas,type_div,long, dataDF) : #ID = 'RSI' ou ID="cours"
    i=0
    liste=[]
    ...
    if type_div=='RegularBearish' :
        if ID=="cours" :
            ...
            while i<len(indM) :
                ...
                for j in range(i,min(i+long,len(indM))) :
                    if l[indM[j]]>l[indM[i]] and not(coupe(indM[i],indM[j],'Close',plusHautBas, dataDF)) :
        ...
        if ID=='RSI' :
            ...
            while i<len(indM):
                ...
                for j in range(i,min(i+long,len(indM))) :
                    if l2[indM[j]]<l2[indM[i]] and not(coupe(indM[i],indM[j],'RSI',plusHautBas, dataDF)) :
...
def creation_divergenceBearReg(Hausses_interessante_cours,Baisse_interessante_rsi, dataDF) : 
    Divergence=[]

    for (x,y) in Hausses_interessante_cours :
        if dataDF['RSI'][y]-dataDF['RSI'][x]<=0 :
            if not(coupe(x,y,'RSI',"Haut", dataDF)) and not((x,y) in Divergence) :
                Divergence.append((x,y))

    for (x,y) in Baisse_interessante_rsi :
        if dataDF['Close'][y]-dataDF['Close'][x]>=0 :
            if not(coupe(x,y,'Close',"Haut", dataDF)) and not((x,y) in Divergence) :
                Divergence.append((x,y))
    return(Divergence)

def creation_divergenceBearHidden(Baisse_interessante_cours,Hausse_interessante_rsi, dataDF) : 
    Divergence=[]
...
def creation_divergenceBullReg(Baisse_interessante_cours,Hausse_interessante_rsi, dataDF) :
    Divergence=[]

    for (x,y) in Baisse_interessante_cours :
        if dataDF['RSI'][y]-dataDF['RSI'][x]>=0 :
            if not(coupe(x,y,'RSI',"Bas", dataDF)) and not((x,y) in Divergence) :
                Divergence.append((x,y))

    for (x,y) in Hausse_interessante_rsi :
        if dataDF['Close'][y]-dataDF['Close'][x]<=0 :
            if not(coupe(x,y,'Close',"Bas", dataDF)) and not((x,y) in Divergence) :
                Divergence.append((x,y))
    return(Divergence)

def creation_divergenceBullHidden(Hausse_interessante_cours,Baisse_interessante_rsi, dataDF) :
    Divergence=[]
...
#renvoit la hauteur du decrochage après la divergence.
#utilisé pour le backtesting > sur des données qu'on connait.
...
def pente_qualite_arthur( divergence, serie):
    """

    Parameters
    ----------
    divergence : 
        Divergence contient le point de début ""a et le pt de fin "b" d'une divergence
    serie: 
        série temporelle ( serie=RSI ou serie=cours)
    Returns
    -------
    La pente de la droite reliant serie(a) et serie(b)

    """
    (a,b)=divergence
    pente=( serie[b]-serie[a] )/(b-a)
    return round(pente, 2)


#renvoi un triplet angle_cours, angle_rsi, hauteur de chute pour une divergence donnée
def triplet_bear_ML(divergence, l, l2):    
    alpha_cours = pente_qualite(divergence, l)
    alpha_ind = pente_qualite(divergence, l2)
    
    h = Effect(divergence,l,'bear',s=100)
    
    return (alpha_cours, alpha_ind, h)

def liste_triplet_ML(list_div, l, l2):
    list = []
    for div in list_div:
        list.append(triplet_bear_ML(div, l, l2))
    return list


def pente_qualite( divergence, cours, RSI):
    """
    Parameters
    ----------
    divergence : 
        Divergence contient le point de début "a" et le point de fin "b" d'une divergence
    Returns
    -------
    La pente de la droite reliant serie(a) et serie(b)
    """
    (a,b)=divergence
    pente1=( cours[b]-cours[a] )/(b-a)
    pente2=( RSI[b]-RSI[a] )/(b-a)
